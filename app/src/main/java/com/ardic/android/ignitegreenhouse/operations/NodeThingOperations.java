...
public class NodeThingOperations {
    private static final String TAG = NodeThingOperations.class.getSimpleName();
    ...
    private static NodeThingOperations INSTANCE = null;

    private NodeThingOperations(Context context) {
        mContext = context;
        if (context != null) {
    ...
    public static synchronized NodeThingOperations getInstance(Context context) {
        if (INSTANCE == null) {
            INSTANCE = new NodeThingOperations(context);
        }
        return INSTANCE;
    }
        ...
        try {
            ...
            for (int nodeNumber = 0; nodeNumber < getAddDataSize; nodeNumber++) {
                ...
                for (int thingNumber = 0; thingNumber < getAddThingSize; thingNumber++) {
                    ...
                    addJsonControl(addDeviceArray.getString(Constant.NODE_ID_JSON_KEY), addThingArray.getString(Constant.THING_CODE_JSON_KEY), addThingArray.getString(Constant.THING_LABEL_JSON_KEY));
                }
            }
    ...
    private void addJsonControl(String getNode, String getThing, String getThingLabel) {
        ...
        if (!sensors.contains(getNode + ":" + getThingLabel) && SensorTypeOperations.getInstance(mContext).getSensorType(getThing) != null) {
            ...
            try {
                ...
                returnCreateTrue.put(Constant.THING_LABEL_JSON_KEY, getThing);
            ...
            try {
                ...
                if (SensorTypeOperations.getInstance(mContext).getSensorType(getThing) == null) {
                    JSONObject returnCreateFalse = new JSONObject().put(RESPONSE_CREATE_MESSAGE_STRING, new JSONObject()
                            .put(RESPONSE_CREATE_MESSAGE_NODE_STRING, false)
                            .put(RESPONSE_CREATE_MESSAGE_THING_STRING, false)
    ...
    public void removeSavedThing(String getNode, String getThingLabel) {
        if (!TextUtils.isEmpty(getNode) && !TextUtils.isEmpty(getThingLabel) && sensors.contains(getNode + ":" + getThingLabel)) {
            ...
            try {
                JSONObject removeThingJson = new JSONObject().put(Constant.REMOVE_THING_STRING_RESPONSE, new JSONObject()
    ...
    public void removeSavedAllThing() {
        ...
        try {
            JSONObject removeAllThing = new JSONObject().put(Constant.REMOVE_THING_STRING_RESPONSE, Constant.REMOVE_ALL_COMPONENT_MESSAGE);
